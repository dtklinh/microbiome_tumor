...
is.valid.batch <- function(batch_num, batch_type, meta_data ){
  ...
  }else if(grepl("sequencing", tolower(batch_type), fixed = TRUE)){
    tmp <- meta_data[meta_data$Seq_round==batch_num, "TRUE_control"] %>% table() %>% length()
  }else{
    stop("Batch type must contain either dna or pcr or sequencing.")
  }
  return(tmp==2)
}
...
Extract_SampleIdx <- function(batch_num, batch_type, meta_data){
  ...
  if(grepl("dna", tolower(batch_type), fixed = TRUE)){
    ...
    sub_meta_data <- meta_data[meta_data$Seq_round == batch_num,]
  }else{
    stop("Batch type must contain either dna or pcr or sequencing.")
  }
...
is.valid.nct.type <- function(batch_num, batch_type, meta_data){
  ...
  #for medium
  n.medium <- length(which(sub_meta_data$sample_type == "medium"))
  n.medium <- !(n.medium == 0)
  n.nct.type["medium"] <- n.medium
  #for sequence
  n.sequence <- length(which(sub_meta_data$sample_type == "sequence"))
  n.sequence <- !(n.sequence == 0)
  n.nct.type["sequence"] <- n.sequence
  return(n.nct.type)
}
...
create.sub.NCT.Sample.type <- function(batch_num,batch_type,meta_data,typeVector,NCT.sample,i){
  ...
  if(grepl("dna", tolower(batch_type), fixed = TRUE)){
    ...
    stop("Batch type must contain either dna or pcr sequencing.")
  }
...
Wrapper_Nejman1 <- function(True.Sample, NCT.Sample, metadata, x){
    ...
    #get batch numbers for Seq
    tmp <- metadata$Seq_round %>% as.numeric()
    Seq_lst <- tmp[!is.na(tmp)] %>% unique()
    ...
    ## Third, we fix the condition Sequencing
    tmp_lst_Seq <- lapply(Seq_lst, BinomTest_Wrapper, batch_type="Sequencing", meta_data=metadata, 
                          True.Sample=True.Sample, NCT.sample=NCT.Sample)
    
    tmp_lst_Seq <- unique(unlist(tmp_lst_Seq))
    
    Lst_contaminant <- union(tmp_lst_DNA,tmp_lst_PCR)
    Lst_contaminant <- union(Lst_contaminant,tmp_lst_Seq)
...
Wrapper_Nejman2 <- function(True.Sample, NCT.Sample, metadata, x){
  #get batch numbers for DNAex
  tmp <- metadata$DNAex_round %>% as.numeric()
  DNAex_lst <- tmp[!is.na(tmp)] %>% unique()
  #get batch numbers for PCR
...
cont_nej_low <- function(True.Sample, NCT.Sample, metadata, x){
  #get batch numbers for DNAex
  tmp <- metadata$DNAex_round %>% as.numeric()
  DNAex_lst <- tmp[!is.na(tmp)] %>% unique()
  #get batch numbers for PCR
...
get_DESeq_dataset <- function(phyloseq){
  library("DESeq2")
  phyloseq <- prune_samples(x = phyloseq,sample_sums(phyloseq) >0) #no empty samples
  
  DESeq2_obj <- phyloseq_to_deseq2(phyloseq, ~ sample_side) #convert phyloseq into DESeq object
  ...
  return(side_dds) # return side_dds
}
  
get_DESeq_result <- function(phyloseq,DESeq_dataobj,alpha = 0.05){
  res <- results(object = DESeq_dataobj) #access results
  res <- res[order(res$padj, na.last=NA),] #order p.adj values
  alpha = alpha #set alpha
  sigtab <- res[(res$padj < alpha),] # table of taxa passing significance threshold
  sigtab <- cbind(as(object = sigtab,Class = "data.frame"), as(tax_table(phyloseq)[rownames(sigtab),],"matrix"))
  return(sigtab)
}


get_DESeq_plot <- function(DESeq_resultobj){  
  library(ggplot2)
  theme_set(theme_bw())
  sigtabgen = subset(DESeq_resultobj, !is.na(species))
  # phylum order
  ...
  tmp_plot <- ggplot(sigtabgen, aes(y=genus, x=log2FoldChange, color=phylum)) + 
...
get_VST_matrix <- function(DESeq_dataobj){  
  VST_matrix <- getVarianceStabilizedData(DESeq_dataobj)
  return(VST_matrix)
}

get_VST_phy_mod <- function(phyloseq, VST_matrix){
  VST_otu <- otu_table(VST_matrix,taxa_are_rows = TRUE)
  VST_sam <- sample_data(phyloseq)
  VST_tax <- tax_table(phyloseq)
  VST_phy <- phyloseq(VST_otu,VST_sam,VST_tax)
  vst_min_p <- abs(min(VST_otu))
  VST_phy_mod <- transform_sample_counts(VST_phy,function(x){x+vst_min_p})
  return(VST_phy_mod)
}