...
beta_analysis_bray <- function(Phyloseq,i = "sample_side", title = "Tissue samples - Bray Curtis"){
...
compositional_analysis_CP <- function(PhyloseqObj, target_rank='order', rel_abundance=0.03, wrap_var = 'sample_side'){
  ## PhyloseqObj: phyloseq object after filtering, decontamination and normalization
  ## target_rank: one of : phylum,order, family, genus, species
  ## rel_abundance: a threshold which less than that to be considered as 'others'.
  physeq.fam <- transform_sample_counts(PhyloseqObj, function(x){x/sum(x)})
  ...
  #cols <- heat.colors((df.fam.melt$target_rank %>% unique() %>% length()), alpha = 1)
  cols <- cols <- c("#9d547c","#56ca63","#a357d6","cornflowerblue","#419d2a","sandybrown","red3","peachpuff","cyan","paleturquoise3","mistyrose","mediumpurple","mediumseagreen","mediumorchid","moccasin","orange4","olivedrab","midnightblue","papayawhip","palevioletred4","brown1","orchid","navy","darkred","navajowhite1","mistyrose1","#525fd6","#8cbe3a","#c944aa","#5ba557","#9e66cb","#c1b735","#6d82ec","#e69728","#6654b0","#799330","#da7fdf","#3c782c","#e44586","blue4","#63c996","#dc3f53","#49cbc8","#cf3f29","#4fabda","#da6c2b","#598bd1","#b78c24","#8d4191","#a0b971","slategray1","sienna","plum1","lightyellow1","lightskyblue3","linen","limegreen","mediumaquamarine","gray14","gold3","darkviolet","#b2386a","#479d71","#ae4341","#2ba198","#e07557","#5361a3","#dda353","#aa98df","#5b6114","#dc89bf","#327243","#e57b94","#277257","#9b62a0","#bbab59","#98495a","#526229","#d8827d","#857624","gray40","#9a4a22","#7c7d46","#e3a073","#9e6b33", "gray74", "lawngreen","gainsboro","dodgerblue3","deeppink3","firebrick3", "orchid2", "olivedrab1", "ivory3", "darkseagreen", "bisque2", "darkgoldenrod2", "blue2", "skyblue", "seashell2", "turquoise", "tan1", "seagreen2", "palevioletred3", "linen", "steelblue4","ghostwhite","dodgerblue1","deeppink1","firebrick1", "limegreen", "purple3", "khaki3", "snow3", "darkslategray", "lavender", "magenta2", "palegreen", "salmon", "maroon", "cyan2","#671408","#FAEBD7","#7FFFD4","#F0FFFF","#A52A2A","burlywood","cadetblue","#7FFF00","chocolate","cornsilk","#FF7F50","#008B8B","darkgoldenrod1","darkolivegreen","darkorange4","white","goldenrod2","darkgreen","oldlace","darkslategray3","navajowhite3","orchid4","gray25","#F0924D")

  if (wrap_var == "sample_side") {
    tmp_plot <- ggplot(df.fam.melt, aes(x=id, y=Abundance, fill=target_rank)) +
    geom_bar(stat="identity") +
    scale_y_continuous(labels = scales::percent, expand = c(0, 0), limits = c(0, 1.02)) +
    scale_fill_manual(values = cols) +
    xlab ("") +
    ylab("Relative abundance") +
    theme_bw() +
    facet_wrap(vars(sample_side), scales = "free")+
    ggtitle(paste0("Microbial composition at ",target_rank, " level"))+
    ...
    theme(legend.position = "bottom",
     ...
     tmp_plot <- ggplot(df.fam.melt, aes(x=id, y=Abundance, fill=target_rank)) +
      ...
      ylab("Relative abundance") +
      theme_bw() +
      facet_wrap(vars(TRUE_control), scales = "free")+
      ggtitle(paste0("Microbial composition at ",target_rank, " level"))+
    ...
    tmp_plot <- ggplot(df.fam.melt, aes(x=id, y=Abundance, fill=target_rank)) +
      ...
      ylab("Relative abundance") +
      theme_bw() +
      facet_wrap(vars(mouse_sex), scales = "free")+
      ggtitle(paste0("Microbial composition at ",target_rank, " level"))+
      theme( axis.text.x = element_text(angle=45,hjust = 1),
             axis.text.y = element_text (size=12),
             axis.title = element_text(size=14, face="bold"))+
  ...
  else if (wrap_var == "mouse_age_type") {
    tmp_plot <- ggplot(df.fam.melt, aes(x=id, y=Abundance, fill=target_rank)) +
      geom_bar(stat="identity") +
      scale_y_continuous(labels = scales::percent, expand = c(0, 0), limits = c(0, 1.02)) +
      scale_fill_manual(values = cols) +
      ...
      ylab("Relative abundance") +
      theme_bw() +
      facet_wrap(vars(mouse_age_type), scales = "free")+
      ggtitle(paste0("Microbial composition at ",target_rank, " level"))+
      theme( axis.text.x = element_text(angle=45,hjust = 1),
             axis.text.y = element_text (size=12),
             axis.title = element_text(size=14, face="bold"))+
    ...
    tmp_plot <- ggplot(df.fam.melt, aes(x=id, y=Abundance, fill=target_rank)) +
      ...
      ylab("Relative abundance") +
      theme_bw() +
      facet_wrap(vars(DNAex_round), scales = "free")+
      ggtitle(paste0("Microbial composition at ",target_rank, " level"))+
      theme( axis.text.x = element_text(angle=45,hjust = 1),
             axis.text.y = element_text (size=12),
             axis.title = element_text(size=14, face="bold"))+
    ...
    tmp_plot <- ggplot(df.fam.melt, aes(x=id, y=Abundance, fill=target_rank)) +
      ...
      ylab("Relative abundance") +
      theme_bw() +
      facet_wrap(vars(PCR_round), scales = "free")+
      ggtitle(paste0("Microbial composition at ",target_rank, " level"))+
      theme( axis.text.x = element_text(angle=45,hjust = 1),
             axis.text.y = element_text (size=12),
             axis.title = element_text(size=14, face="bold"))+
